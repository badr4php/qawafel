...
class DataProviderX implements DataProviderInterface
{
    ...
    private function filterByStatus($request){
        if($request->filled('statusCode') && array_key_exists($request->get('statusCode'), self::STATUS_CODE)){
            $this->collection = $this->collection->where('transactionStatus', self::STATUS_CODE[$request->get('statusCode')]);
        }
    }

    private function filterByAmount($request){
        if($request->filled('amounteMin')){
            $this->collection = $this->collection->where('transactionAmount', '>=', $request->get('amounteMin'));
        }
        if($request->filled('amounteMax')){
            $this->collection = $this->collection->where('transactionAmount', '<=', $request->get('amounteMax'));
        }
    }
